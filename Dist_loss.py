...
class DistLoss(nn.Module):
    ...
    def forward(self, inp, tar, theoretical_labels):
        ...
        sorted_inp = sorted_inp.reshape(-1, 1).to(device)
        ...
        slop_loss = slope_regularizer(inp, tar) * 100
        
        # Compute the total loss
        total_loss = self.loss_weight * dist_loss + plain_loss + slop_loss
        
        # Return the average or sum of the loss based on size_average
        if self.size_average:
            if self.require_loss_values:
                return total_loss.mean(), dist_loss.mean(), plain_loss.mean(), slop_loss.mean()
            else:
                return total_loss.mean()
        else:
            if self.require_loss_values:
                return total_loss.sum(), dist_loss.sum(), plain_loss.sum(), slop_loss.sum()
            else:
...
def slope_regularizer(pred, y):
    # pred, y: [N,1]
    cov = torch.mean((pred - pred.mean()) * (y - y.mean()))
    var = torch.mean((pred - pred.mean())**2)
    slope = cov / (var + 1e-6)
    return (slope - 1.0)**2   # 希望斜率=1
