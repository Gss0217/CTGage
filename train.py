...
# 数据归一化
all_train_points = np.concatenate(train_fhr_filter)
train_mean = all_train_points.mean()
train_std = all_train_points.std()
train_fhr_norm = (train_fhr_filter - train_mean) / (train_std + 1e-6)

all_val_points = np.concatenate(val_fhr_filter)
val_mean = all_val_points.mean()
val_std = all_val_points.std()
val_fhr_norm = (val_fhr_filter - val_mean) / (val_std + 1e-6)
